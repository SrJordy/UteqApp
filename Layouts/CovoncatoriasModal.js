import ConvocatoriaScreen from "./CrearCovoncatorias";
import GestionCovon from './GestionCovoncatoria';
...
const ModalCovoncatorias = ({ isVisible, onClose }) => {
  const [openCovoncatorias, setOpenCovoncatorias] = useState(false);
  const [showBackButton, setShowBackButton] = useState(false);
  const [showCloseButton, setShowCloseButton] = useState(true);
  const [openCovoncatoriasG, setCovoncatoriasG] = useState(false);
  const navigation = useNavigation();

  const handleCardPress = (title) => {
    if (title === 'Crear') {
      //llamar a la interfaz de cotenido mediente por navigation y cierra el modal 
      navigation.navigate("Covoncatorias");
      isVisible=onClose();
      console.log('Abrir Crear');
    } else if (title === 'Gestionar') {
      navigation.navigate("CovoncatoriasG");
      isVisible=onClose();
  ...
  const handleBackButtonPress = () => {
    setOpenCovoncatorias(false);
    setCovoncatoriasG(false);
  ...
  return (
    <Modal visible={isVisible} animationType="slide" transparent>
      <View style={stylesModalC.modalContainer}>
        {!openCovoncatorias && !openCovoncatoriasG ? (
          <View style={stylesModalC.modalContent}>
            <Text style={stylesModalC.modalTitle}>Convocatorias</Text>
            <FlatList
              data={[
          ...
          <View style={[stylesModalC.modalContent, openCovoncatorias || openCovoncatoriasG ? stylesModalC.expandedModal : null]}>
            {openCovoncatorias ? (
              <ConvocatoriaScreen />
            ) : (
              <GestionCovon />
...
export default ModalCovoncatorias;
